...
The project deliberately separates **At Risk** from **Churned** so that the analysis distinguishes an early retention opportunity from a longer-term inactive customer.

- **Active:** customer has at least one delivered order within the most recent 90 days of the dataset observation period.
- **At Risk:** customer has previously ordered but has no delivered order for 60–89 days. This is an early warning segment, not permanent churn.
- **Churned:** customer has previously ordered but has been inactive for 90+ days.
- **Never Ordered:** registered customer with no delivered-order history. These customers are excluded from retention calculations because they were never retained customers.

The SQL inactivity analysis uses the **60-day threshold as an at-risk indicator**. The 90-day threshold is used for the dashboard's active/churned customer classification. Keeping these thresholds explicit prevents the 60-day risk check from being incorrectly described as a churn definition.
...
14. Why did you choose a 90-day active/churn threshold?
15. Why is the 60-day inactivity check treated as At Risk rather than Churned?
16. Why keep `At Risk` separate from `Churned`?
17. Why are `Never Ordered` customers excluded from the retention denominator?
18. How did you calculate the retention rate?
19. What action would you take if churn is concentrated in one city?

### Power BI & DAX

20. Why use measures instead of calculated columns for KPIs?
21. Why use `DISTINCTCOUNT` for customers?
22. How does filter context affect your retention measure?
23. How would a city or membership slicer change the KPI cards?
24. Why did a retention calculation produce unexpected results during development?

### Business insights

25. Which finding would you present first to management?
26. What is the most important churn-reduction recommendation?
27. What operational issue does the delivery page reveal?
28. How can restaurants use the performance page?
29. Which KPI would you monitor weekly after implementing your recommendations?
30. What additional data would you request in a real Zomato environment?
